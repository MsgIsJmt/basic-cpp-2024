...
void searchBook(int choose, char* keyword) {
    struct Book* current = head;

    while (current != NULL) {
        if (choose == 1) {
            strcmp(current->title, keyword) == 0;
            printf("도서 제목: %s, 작가: %s, 출판사: %s, 도서 ID: %d, 대여자: %s\n",
                current->title, current->author, current->publisher, current->id, current->renter);
        }
        else if (choose == 2) {
            strcmp(current->author, keyword) == 0;
        }
        else if (choose == 3) {
            strcmp(current->publisher, keyword) == 0;
        }
        else if (choose == 4) {
            current->id == atoi(keyword);
        }
        else if (choose == 5) {
            strcmp(current->renter, keyword) == 0;
        }
        else if (choose > 5) {
            printf("메뉴안의 숫자만");
        }
